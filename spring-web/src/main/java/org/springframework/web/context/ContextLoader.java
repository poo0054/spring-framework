 ...
 * WEB-INF/applicationContext2.xml". Ant-style path patterns are supported as
 * well, e.g. "WEB-INF/*Context.xml,WEB-INF/spring*.xml" or
...
public class ContextLoader {
	 ...
	 * for initializing all web application contexts in the current application:
	 * {@value}.
	 * 
	 * @see #customizeContext(ServletContext, ConfigurableWebApplicationContext)
	 */
	...
	/**
	 * Name of the class path resource (relative to the ContextLoader class) that
	 * defines ContextLoader's default strategy names.
	 */
	...
	/**
	 * The 'current' WebApplicationContext, if the ContextLoader class is deployed
	 ...
	 * based on the "contextClass" and "contextConfigLocation" servlet
	 * context-params. See class-level documentation for details on default values
	 * for each.
	 * <p>
	 * This constructor is typically used when declaring the {@code
	 * ContextLoaderListener} subclass as a {@code <listener>} within
	 ...
	 * subclasses are free to call the {@link #closeWebApplicationContext} method on
	 ...
	 * specified through the "contextInitializerClasses" init-param will be
	 * applied.</li>
	 ...
	 * using the application context provided at construction time, or creating a
	 * new one according to the "{@link #CONTEXT_CLASS_PARAM contextClass}" and
		...
		try {
			...
			if (this.context instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) {
					...
					// 配置并初始化 IoC容器，看到下面方法中的 Refresh单词 应该能想到 AbstractApplicationContext 中的
					// refresh()方法，猜到它是前面介绍的 IoC容器 的初始化入口
	...
	protected WebApplicationContext createWebApplicationContext(ServletContext sc) {
		...
		//默认 XmlWebApplicationContext 或自定义上下文类（如果指定）。
	 ...
	 * ContextLoader after config locations have been supplied to the context but
	 * before the context is <em>refreshed</em>.
	 * <p>
	 * The default implementation
	 * {@linkplain #determineContextInitializerClasses(ServletContext) determines}
	 * what (if any) context initializer classes have been specified through
	...
	/**
	 * Return the {@link ApplicationContextInitializer} implementation classes to
	 * use if any have been specified by {@link #CONTEXT_INITIALIZER_CLASSES_PARAM}.
	 * 
	 * @param servletContext
	 *            current servlet context
	 * @see #CONTEXT_INITIALIZER_CLASSES_PARAM
	 ...
	 * for the current thread's context ClassLoader, which needs to be the web
