...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
     ...
     * 
     * 提前的bean,比如xml加载的 或者自定义的BeanDefinition
     */
                    ...
                    if (!mbd.isAbstract()
                        ...
                        if (!isFactoryBean) {
                            if (includeNonSingletons || isSingleton(beanName, mbd, dbd)) {
                                // 用于检查具有给定名称的 bean 是否与指定类型匹配。允许应用额外的约束以确保不会提前创建 bean。
    ...
    public BeanDefinition getBeanDefinition(String beanName) throws NoSuchBeanDefinitionException {
        // beanDefinitionMap是解析xml时候就已经放入了
        BeanDefinition bd = this.beanDefinitionMap.get(beanName);
        if (bd == null) {
            if (logger.isTraceEnabled()) {
    ...
    public void preInstantiateSingletons() throws BeansException {
        ...
        for (String beanName : beanNames) {
            ...
            RootBeanDefinition bd = getMergedLocalBeanDefinition(beanName);
            // 返回此 bean 是否是“抽象的”，即不打算实例化自身，而只是用作具体子 bean 定义的父级。
            if (!bd.isAbstract() && bd.isSingleton() && !bd.isLazyInit()) {
                if (isFactoryBean(beanName)) {
                    // 在beanName前面添加字符串 获取bean
    ...
    public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
        ...
        // 从容器中获取并进行判断 如果存在 并且 allowBeanDefinitionOverriding（是否应允许通过注册具有相同名称的不同定义来覆盖 bean 定义）不为true 抛出异常
        BeanDefinition existingDefinition = this.beanDefinitionMap.get(beanName);
        if (existingDefinition != null) {
            // 如果能进行覆盖 就不报错 打印log日志
            ...
            if (hasBeanCreationStarted()) {
                ...
                // 仍处于启动注册阶段 可能是创建beanFactory的时候 加载xml文件 然后写入BeanMap
                this.beanDefinitionMap.put(beanName, beanDefinition);
